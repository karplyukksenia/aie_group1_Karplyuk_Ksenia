...
class ColumnSummary:
    ...
    zero_count: Optional[int] = None # новое
...
def summarize_dataset(
    ...
    for name in df.columns:
        ...
        zero_count: Optional[int] = None  # НОВОЕ ПОЛЕ
        ...
        if is_numeric and non_null > 0:
            ...
            std_val = float(s.std())
            # Считаем количество нулевых значений среди непустых
            zero_count = int((s.dropna() == 0).sum())
        ...
        columns.append(
            ColumnSummary(
                ...
                mean=mean_val,
                std=std_val,
                zero_count=zero_count,  # новое поле
            )
        )
...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    def has_constant_columns(summary: DatasetSummary) -> bool:
        """
        Проверяет, есть ли в колонки с постоянными значениями.
        Возвращает True, если найдена хотя бы одна колонка.
        """
        for col in summary.columns:
            # если все непустые значения одинаковые
            if col.unique == 1 and col.non_null > 0:
                return True
        return False

    def has_high_cardinality_categoricals(summary: DatasetSummary) -> bool:
        """
        Проверяет, есть ли категориальные колонки с очень большим числом уникальных значений.
        Использует порог по умолчанию: 100 уникальных значений.
        """
        cardinality_threshold = 100
        for col in summary.columns:
            if col.dtype in ['object', 'category', 'string']:
                if col.unique > cardinality_threshold:
                    return True
        return False

    def has_many_zero_values(summary: DatasetSummary) -> bool:
        """
        Проверяет, есть ли числовые колонки с высокой долей нулевых значений.
        Порог: 70% нулей среди непустых значений.
        """
        zero_threshold = 0.7

        for col in summary.columns:
            if col.is_numeric and col.zero_count is not None and col.non_null > 0:
                zero_ratio = col.zero_count / col.non_null
    ...
    flags["constant_columns"] = has_constant_columns(summary)
    flags["high_cardinality_categoricals"] = has_high_cardinality_categoricals(summary)
    flags["many_zero_values"] = has_many_zero_values(summary)
